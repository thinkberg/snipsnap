...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<c:out value='${app.configuration.locale}'/>" xml:lang="<c:out value='${app.configuration.locale}'/>">
 ...
 <body>
  <div id="page-logo">
   <c:choose>
    <c:when test="${snip.name=='start'}"><s:image name="${app.configuration.logo}"/></c:when>
    <c:otherwise><a href="<c:out value='${app.configuration.url}'/>" accesskey="1"><s:image name="${app.configuration.logo}"/></a></c:otherwise>
   </c:choose>
  </div>
  <div id="page-title">
